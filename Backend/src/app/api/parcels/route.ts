...
import { createParcelSchema } from '@/application/dtos'
...
export const GET = createAuthHandler(
  async (request) => {
    try {
      ...
      const parcels = await parcelService.findByFarmer(farmerId)
...
export const POST = createAuthHandler(
  async (request) => {
    try {
      ...
      const result = await parcelService.create(validated)
