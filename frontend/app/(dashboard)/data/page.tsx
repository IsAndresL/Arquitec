...
import { CropObservation, FarmParcel } from "@/types";
import { 
  Plus, 
  ArrowLeft, 
  ChevronRight,
  ...
  return (
    <div className="h-full relative overflow-y-auto" style={{ backgroundColor: COLORS.white }}>
      ...
      <div className="p-4 flex items-center" style={{ backgroundColor: COLORS.purple.primary }}>
        <button 
          onClick={() => step === "list" ? window.history.back() : setStep("list")} 
        ...
        <h2 className="text-xl flex-1" style={{ color: COLORS.white, fontWeight: 700 }}>
          {step === "list" ? "Observaciones" : "Nueva Observación"}
        </h2>
      </div>
            ...
            <div className="space-y-3">
              {parcels.map((parcel) => (
                <button 
                  ...
                  className="w-full p-5 rounded-2xl flex items-center gap-3 transition-all border-2 border-gray-100 bg-white hover:border-purple-300 active:scale-[0.98]"
                >
                  <div className="w-12 h-12 bg-purple-50 rounded-xl flex items-center justify-center">
                    <Sprout size={24} color={COLORS.purple.primary} />
                  </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            <div className="p-4 bg-purple-50 rounded-2xl flex items-center gap-3 border border-purple-100">
               <div className="w-10 h-10 bg-white rounded-lg flex items-center justify-center">
                  <Sprout size={20} color={COLORS.purple.primary} />
               </div>
               <div>
            ...
            <div>
              ...
              <div className="grid grid-cols-2 gap-3">
                {[
                  { id: 'SANO', label: 'Sano', icon: CheckCircle2, color: '#22c55e' },
                  { id: 'HOJAS_AMARILLAS', label: 'Hojas Amarillas', icon: AlertCircle, color: '#f59e0b' },
                  { id: 'MANCHAS_NEGRAS', label: 'Manchas Negras', icon: AlertOctagon, color: '#374151' },
                  { id: 'HOJAS_SECAS', label: 'Hojas Secas', icon: Leaf, color: '#b45309' }
                ].map((s) => {
                  const Icon = s.icon;
                  return (
                    <button
            ...
            <div>
              <label className="block mb-2 text-xs font-bold uppercase tracking-wider text-gray-400">Notas</label>
              <textarea 
                value={newObservation.notes}
                onChange={e => setNewObservation({...newObservation, notes: e.target.value})}
                placeholder="¿Qué ves en el cultivo?"
                className="w-full p-4 rounded-xl bg-gray-50 border-2 border-gray-100 focus:border-purple-500 focus:outline-none transition-all h-32 text-gray-900 font-medium"
              />
            </div>

            <div className="flex gap-3">
              <button type="button" onClick={() => setStep("list")} className="flex-1 p-4 rounded-xl font-bold text-gray-700 bg-gray-100 hover:bg-gray-200 transition-colors">Cancelar</button>
              <button type="submit" className="flex-[2] p-4 rounded-xl flex items-center justify-center gap-2 active:scale-95 transition-transform" style={{ backgroundColor: COLORS.purple.primary, color: COLORS.white, fontWeight: 700 }}>
                <CheckCircle size={20} />
                <span>Guardar</span>
              </button>
            </div>
